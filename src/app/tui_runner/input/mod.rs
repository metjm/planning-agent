...
pub mod mouse_input;
pub mod session_browser_input;
pub mod workflow_browser_input;

use approval_input::{
    handle_awaiting_choice_input, handle_entering_feedback_input, handle_entering_iterations_input,
};
...
pub async fn handle_key_event(
    ...
    if tab_manager.session_browser.open {
        should_quit = session_browser_input::handle_session_browser_input(
            key,
            tab_manager,
            working_dir,
            output_tx,
    ...
    if tab_manager.workflow_browser.open {
        should_quit =
            workflow_browser_input::handle_workflow_browser_input(key, tab_manager, output_tx)
                .await?;
        return Ok(should_quit);
    }
...
async fn handle_approval_mode_input(
    ...
    match session.approval_mode {
        ...
        ApprovalMode::EnteringIterations => handle_entering_iterations_input(key, session).await,
